...
page_assets = create_assets_from_dir("./pages")
...
async def webtoken_for_localhost(request):
    ...
    if "username" not in request.querydict or "password" not in request.querydict or "totp" not in request.querydict:
        return 403, {}, "parameter missing"

    username = request.querydict["username"]
    password = request.querydict["password"]
    totp = request.querydict["totp"]

    if username not in USERS:
        return 403, {}, "username not fount"

    if USERS[username]["password"] != password:
        return 403, {}, "password incorrect"

    if pyotp.TOTP(USERS[username]["totp_secret"]).verify(totp) is False:
        return 403, {}, "totp incorrect"
...
if __name__ == "__main__":
    asgineer.run(main_handler, "uvicorn", "0.0.0.0:7080", log_level="warning")
